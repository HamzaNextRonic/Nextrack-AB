...
public class HomeController implements Initializable {
    ...
    private String StartTime = "MM-dd-yyyy 00:00:00";
    private String StopTime = "MM-dd-yyyy 00:00:00";
    ...
    private Table createContent(PDRectangle PAGE_SIZE) {
        ...
        String date;
        if(english){
             date = choosedDate.format(DateTimeFormatter.ofPattern("MM-dd-yyyy"));
        }else{
            date = choosedDate.format(DateTimeFormatter.ofPattern("yyyy-MM-dd"));
        }

        int idD = getIdDevice(serial);
        System.out.println("choosedDate " + choosedDate + "date = " + date);

        String query = "select * from events where id_device =" + idD + " and  time >= '" + date + " 00:00:01" + "' and time <=  '" + date + " 23:59:59" + "'  ";
        int count = 0;
        try {
            PreparedStatement preparedStatement = null;
            preparedStatement = connection.prepareStatement(query);
    ...
    private Table createContentListWarning(PDRectangle PAGE_SIZE) {
        ...
        String query = "select * from warnning where id_device = " + idD + " and  time >= '" + date + " 00:00:01" + "' and time <=  '" + date + " 23:59:59" + "'  ";
        int count = 0;
        try {
            PreparedStatement preparedStatement = null;
            preparedStatement = connection.prepareStatement(query);
    ...
    private Table createContentWarning(PDRectangle PAGE_SIZE) {
        ...
        try {
            ...
            content[1][3] = String.valueOf(2 * countMin) + " minutes";
            if (countMin == 0)
                content[1][4] = "OK";
            else
                content[1][4] = "NON";
    ...
    private Table createContentLogging(PDRectangle PAGE_SIZE) {
        ...
        try {
            ...
            if(StartTime.equals("MM-dd-yyyy 00:00:00")){
                Date WarningFirstSave;
                Date EventFirstSave;
                if(english) {
                    WarningFirstSave = new SimpleDateFormat("MM-dd-yyyy HH:mm:ss").parse(getWarningFirstSave(serial));
                     EventFirstSave = new SimpleDateFormat("MM-dd-yyyy HH:mm:ss").parse(getEventsFirstSave(serial));
                }
                else {
                    WarningFirstSave = new SimpleDateFormat("yyyy-MM-dd HH:mm:ss").parse(getWarningFirstSave(serial));
                    EventFirstSave = new SimpleDateFormat("yyyy-MM-dd HH:mm:ss").parse(getEventsFirstSave(serial));
                }
                ...
                content[0][0] = StartTime;
            }
            //content[0][1] = getevLastSave(serial);
            if(StopTime.equals("MM-dd-yyyy 00:00:00")) {
                Date WarningFirstSave;
                Date EventFirstSave;
                if (english){
                    WarningFirstSave = new SimpleDateFormat("MM-dd-yyyy HH:mm:ss").parse(getWarningLastSave(serial));
                    EventFirstSave = new SimpleDateFormat("MM-dd-yyyy HH:mm:ss").parse(getEventsLastSave(serial));
                }else{
                    WarningFirstSave = new SimpleDateFormat("yyyy-MM-dd HH:mm:ss").parse(getWarningLastSave(serial));
                    EventFirstSave = new SimpleDateFormat("yyyy-MM-dd HH:mm:ss").parse(getEventsLastSave(serial));
                }
                if(WarningFirstSave.after(EventFirstSave)){
    ...
    private void selectUsers() {
        ...
        try {
            ...
            //  new SimpleDateFormat("MM-dd-yyyy HH:mm:ss").parse(
            // getViews(pnlUsers);
...
//            String query = "Update device set  name = '" + updateName_sensor.getText() + "', description = '"
//                    + updateAriaDescription.getText() + "' where serial = '" + upstatednum_serial_id.getText() + "';";
...
//            System.out.println("idDevice = " + serialNumber);
    ...
    public void addDev(ActionEvent event) {
        ...
        s.getIcons().add(new Image(this.getClass().getResource("/app/images/icon.png").toString()));
        //alert.setContentText("êtes-vous sûr de vouloir supprimer cet élément?");
        alert.setContentText("Veuillez appuyer sur \"Récupérer les enregistrements\" pour ajouter un enregistreur");
    ...
    public void DrawChart() throws ParseException {
        ...
        String r = "select Min(temperature) as minval , Max(temperature) as maxval ,sum(temperature) as sum ,count(temperature) as nbpointE from events where id_device = "
                + id + " and (time >='"
                + selectedDateF + " 00:00:00' and time <= '"
                + selectedDateF + " 23:59:59')";
 ...
 */
        ...
        String query = "select * from events " +
                ...
                + selectedDateF + " 00:00:00' and time < '"
                + selectedDateF + " 23:59:59')";
        System.out.println("selectedDate.getMonthValue()= " + selectedDate.getMonthValue());
                                ...
                                if (!a.contains("@") && !a.contains("!") && start) {
                                    try {
                                        ...
                                        if (j == 0) {
                                            ...
                                            boolean bDate = parseDate(date, "MM-dd-yyyy");
                                            if (!bDate) continue;
                                        }
                                        if (j == 1) {
                                        ...
                                        if (j == 2) {
                                            ...
                                            temperature = a.trim();
                                            if (Float.parseFloat(temperature) == -127 || temperature.equals(-127.00))
                                                continue;
...
//                                              System.out.println("bTime= "+bTime);
                                                ...
                                                Date lastDate = new SimpleDateFormat("MM-dd-yyyy HH:mm:ss").parse(d);
                                                System.out.println("lastDate  " + lastDate);
                                                System.out.println("timeeeee " + time);
                                                Date currentDate = new SimpleDateFormat("MM-dd-yyyy HH:mm:ss").parse(date + " " + time);
                                                ...
                                                if (getAllDevices().contains(serialNumber) && currentDate.after(lastDate)) {//
                                                    try {
                                                        ...
                                                        String query = "insert into events (temperature,Time,id_device,record_ID,valiseId) values ("
                            ...
                            for (String a : parts) {
                                ...
                                if (!a.contains("@") && !a.contains("!") && start) {
                                    ...
                                    if (j == 3) {
                                        ...
                                        // System.out.println("j=3"+a.trim());
                                        String d = getWarningLastSave(serialNumber);
                                        // System.out.println("getDateLastSave()  " + d);
                                        Date lastDate = new SimpleDateFormat("MM-dd-yyyy HH:mm:ss").parse(d);
                                        // System.out.println("lastDate  " + lastDate);
                                        //System.out.println("timeeeee " + time);
                                        Date currentDate = new SimpleDateFormat("MM-dd-yyyy HH:mm:ss").parse(date + " " + time);
                                        ...
                                        // System.out.println("+id+ " + getAllDevices().contains(serialNumber));
                                        System.out.println("+date+ " + currentDate.after(lastDate));
                                        if (getAllDevices().contains(serialNumber) && currentDate.after(lastDate)) {//
                                            try {
    ...
    public void InsertEvent() {
        try {
            for (int i = 0; i < event1.size(); i++) {
                ...
                String query = "insert into events (temperature,Time,id_device,record_ID,valiseId) values (" + event1.get(i).getTemperature()
...
//              fileOut.close();
    ...
    public void changeTime() throws InterruptedException {
        if (is_firstTime) {
            ...
            if (chosenPort.openPort()) {
                ...
                System.out.println("MM-dd-yyyy'T'HH:mm:ss " + date);
            ...
            System.out.println("else is_firstTime " + chosenPort.openPort());
            chosenPort.setComPortTimeouts(SerialPort.TIMEOUT_SCANNER, 100, 100);
            chosenPort.setBaudRate(115200);
            String stringOut = "#CMD_TIME!" + "\n";
            chosenPort.writeBytes(stringOut.getBytes(), stringOut.getBytes().length);
    ...
    public void openFile() {
        try {
            File f = new File(pathDisk.toString());
            //File f = new File("D:\\AB10C100.txt");
            System.out.println("AbsolutePath = " + f.getAbsolutePath());
            sc = new Scanner(f, StandardCharsets.ISO_8859_1.name());//iso-8859-1
        } catch (Exception e) {
        ...
        Thread thread = new Thread() {
            ...
            public void run() {
                ...
                String date = " ", time = " ", temperature = " ";
                 System.out.println("pathDisk.tostring "+ pathDisk);
                if(pathDisk !=null){
                    overlayDataTuto.setVisible(false);
                ...
                while (sc.hasNextLine()) {
                    //System.out.println("hasNet ");
                    String line = sc.nextLine();
                    String[] partie = line.split("	", 6);
                    if(cmptLine == 1 && line.contains("DATA REPORT")) english = true;
                    if (cmptLine == 5) {

                        serialNumber = partie[2];
                    ...
                    //System.out.println("line "+line+ "cmpt "+cmptLine);
                    if( english){
                        System.out.println( "cmpt "+cmptLine);
                    if (cmptLine == 21) {

                            String[] part = line.split("	", 8);
                            for (String a : part)
                                System.out.println("a= " + a);

                            System.out.println("part0" + part[7].trim());
                            StartTime = part[7].trim();
                            Highest = part[3].split(" ", 2)[0].trim();
                    ...
                    if (cmptLine == 22) {
                        ...
                        StopTime = part[7].trim();
                        Lowest = part[3].split(" ", 2)[0].trim();
                    ...
                    if (cmptLine == 23) {
                        ...
                        if (cmptLine == 21) {
                            ...
                            System.out.println("part0" + extractDate(part[5].trim()));
                            StartTime = extractDate(part[5].trim());
                            Highest = part[2].split(" ", 2)[0].trim();
                            System.out.println("Highest = " + Highest);
                        ...
                        if (cmptLine == 22) {
                            ...
                            StopTime = extractDate(part[5].trim());
                            Lowest = part[2].split(" ", 2)[0].trim();
                            System.out.println("Lowest = " + Lowest);
                        }
                        if (cmptLine == 23) {
                            String[] part = line.split("	", 8);
                            ...
                            //StopTime = part[7].trim();
                            Average = part[2].split(" ", 2)[0].trim();

                            System.out.println("Average = " + Average);
                        }

                    }
                    if (cmptLine >= 34 && english) {


                        System.out.println("overlayDataTuto " + overlayDataTuto.isVisible());
//                try {
//                    Thread.sleep(10);
//                } catch (InterruptedException e) {
//                    e.printStackTrace();
...
//                overlayDataLoading.setVisible(true);
                        ...
                        String d = "";
                        if (Float.parseFloat(temperature) >= 8 || Float.parseFloat(temperature) <= 2) {
                            d = getWarningLastSave(serialNumber);
                        } else {
                            d = getDateLastSave(serialNumber);
                        }
                        Date lastDate = null;
                        try {
                            lastDate = new SimpleDateFormat("MM-dd-yyyy HH:mm:ss").parse(d);
                            System.out.println("lastDate  " + lastDate);
                            System.out.println("timeeeee " + time);
                            Date currentDate = new SimpleDateFormat("MM-dd-yyyy HH:mm:ss").parse(date + " " + time);
                            System.out.println("currentDate  " + currentDate);
...
//                overlayDataLoading.setVisible(true);
                        ...
                        for (String a : parts)
                            System.out.println("a " + a);
                        System.out.println("parth " + parts.length);
                        date = parts[2].trim().replace("/", "-");
                        System.out.println("date " + date);
                        time = parts[3].trim();
                        System.out.println("time " + time);
                        temperature = parts[5].trim().split(" ")[0];
                        System.out.println("temperature " + temperature);
                        String d = "";
                        if (Float.parseFloat(temperature) >= 8 || Float.parseFloat(temperature) <= 2) {
                            d = getWarningLastSave(serialNumber);
...
// for each pathname in pathname array
        for (File path : paths) {
            ...
            System.out.println("to string: " + fsv.getFiles(path, true).length);
            if (fsv.getFiles(path, true).length == 1) {
                for (File f : fsv.getFiles(path, true)) {
                    System.out.println("files " + f);
                    if (f.getName().contains(".txt")) {
                        pathDisk = f;
                        System.out.println("pathDisk" + pathDisk);
                    }
                }
    ...
    public String extractDate(String dateToUpdate){
        //2021Äê02ÔÂ01ÈÕ 15:42:29

        int cmpt=0;
        String year = dateToUpdate.substring(0,4);
        String time = dateToUpdate.split(" ")[1];
        int i=4;
        while(!Character.isDigit(dateToUpdate.charAt(i))){
            cmpt++;
            i++;
        }
        String month =  dateToUpdate.substring(i,i+2);//4+cmpt,6+cmpt
        ...
        String day = dateToUpdate.substring(i,i+2);
